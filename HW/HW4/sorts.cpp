...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    {
        int temp = i;
        while ((temp > 0) && comp (s[temp], s[temp - 1]))
        {
...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    sort(ptrAuxStudents.begin(), ptrAuxStudents.end(), compareStudentPtr);
      ...
      //        order.
    for(int j = 0; j < ptrAuxStudents.size(); j++)
    {
        students.at(j) = *ptrAuxStudents.at(j);
    }
